...
let menorFaturamento = Math.min(...faturamentoDiario.map(({ valor }) => valor));

let maiorFaturamento = Math.max(...faturamentoDiario.map(({ valor }) => valor));

let mediaFaturamento = faturamentoDiario.reduce((total, { valor }) => total + valor, 0) / faturamentoDiario.length;
